...
from keras.layers import Dense, Dropout, LSTM, BatchNormalization, Input, PReLU, Conv1D, Lambda, \
...
cnn_mod = ReLU(name='LeakyReLU_01')(cnn_mod)
print(cnn_mod.shape)
...
lstm_mod = LSTM(200, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, name='LSTM_200')(input_shape)
print(lstm_mod.shape)
...
lstm_mod = LSTM(100, return_sequences=False, dropout=0.1, recurrent_dropout=0.1, name='LSTM_100')(lstm_mod)
print(lstm_mod.shape)
...
print(lstm_mod.shape)
lstm_mod = UpSampling1D(size=4, name='UpSampling_3')(lstm_mod) #Only needed if GlobalAveragePooling was done above
#lstm_mod = Flatten()(lstm_mod)
print(cnn_mod.shape, lstm_mod.shape)
...
plot_model(model, to_file='CASE-CNN-AttentiveLSTM-02-2.png', show_shapes=True, show_layer_names=True, dpi=300)
...
# fit network
history = model.fit(train_X, train_y , epochs=400, batch_size=batch_size, shuffle=False,
